...
class _EditPaymentScreenState extends ConsumerState<EditPaymentScreen> {
  ...
  String?
  _propertyType; // Store property type to know if it's rental or offplan
  num?
  _totalPrice; // Store total price for OffPlan to calculate remaining amount
  ...
  void _updateRemainingAmount() {
    ...
    if (_propertyType != null &&
        (_propertyType!.toLowerCase().contains('off') ||
  ...
  Future<void> _calculateRemainingAmount() async {
    try {
      ...
      final allPayments = await paymentsService.fetchPaymentsByOccupant(
        widget.occupantRecordId,
      );
      ...
      try {
        ...
        if (_propertyType != null &&
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Amount exceeds remaining balance. Remaining: AED ${_remainingAmount!.toStringAsFixed(0)}',
              ),
              backgroundColor: Colors.red,
            ),
          );
        ...
        final success = await ref
            ...
            .updatePayment(
              ...
              status:
                  _selectedStatus ??
                  'due', // Allow "due" status for saving details
              ...
              propertyType:
                  _propertyType, // Pass property type to determine emi vs rent
        ...
        if (mounted) {
          if (success) {
            ...
            ref.invalidate(
              customerDetailsProvider(widget.occupantRecordId.toString()),
            );
    ...
    try {
      ...
      if (isOffPlan) {
        ...
        if (newPrice > 0) {
          ...
          debugPrint(
            '💰 [UPDATE_PROPERTY_PRICE] Updating OffPlan price: $currentPrice -> $newPrice (diff: $amountDifference)',
          );
        }
      } else {
        ...
        if (newRent > 0) {
          ...
          debugPrint(
            '💰 [UPDATE_PROPERTY_PRICE] Updating Rental rent: $currentRent -> $newRent (diff: $amountDifference)',
          );
        }
      }

      if (updateData.isNotEmpty) {
        final updateSuccess = await occupantsService.updateOccupantRecord(
          widget.occupantRecordId,
          updateData,
        );
        if (updateSuccess) {
          debugPrint(
            '✅ [UPDATE_PROPERTY_PRICE] Property price updated successfully',
          );
        } else {
          debugPrint(
            '❌ [UPDATE_PROPERTY_PRICE] Failed to update property price',
          );
        }
      }
    } catch (e) {
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              currency.format(
                                widget.amount > 0 ? widget.amount : 0,
                              ),
                              style: GoogleFonts.inter(
                                color: Colors.white,
                                fontSize: 18,
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        validator: (value) {
                          ...
                          if (_propertyType != null &&
                              (_propertyType!.toLowerCase().contains('off') ||
                                  _propertyType!.toLowerCase().contains(
                                    'plan',
                                  )) &&
                              _remainingAmount != null &&
    ...
    return Align(
      ...
      child: Container(
        ...
        child: ElevatedButton(
          ...
          child: Text(
            _selectedStatus?.toLowerCase() == 'paid'
                ? 'Mark as Paid'
...
class _PaymentProofUploadField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isImage =
        proofUrl != null && proofUrl!.isNotEmpty && _isImage(proofUrl!);
