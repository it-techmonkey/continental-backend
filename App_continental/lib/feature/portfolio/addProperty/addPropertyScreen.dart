...
class _AddPropertyScreenState extends ConsumerState<AddPropertyScreen> {
  ...
  bool _isLoadingProperty =
      false; // Flag to prevent clearing form during property load
  ...
  Future<void> _loadPropertyData() async {
    ...
    final property = await occupantsService.fetchOccupantDetail(
      widget.occupantRecordId!,
    );
      ...
      if (mounted) {
        setState(() {
          ...
          _otherChargesController.text =
              property.otherCharges?.toString() ?? '';
          ...
          if (mode == PropertyMode.rental) {
            ...
            _selectedRentFrequency = _mapPaymentFrequencyFromBackend(
              property.paymentFrequency,
            );
            _paymentCountController.text =
                property.paymentCount?.toString() ?? '';
            ...
            if (property.handover != null) {
              _handoverController.text = DateFormat(
                'yyyy-MM-dd',
              ).format(property.handover!);
  ...
  String? _mapFurnishingFromBackend(String? f) {
    ...
    if (v == 'kitchen_appliances_only')
      return 'Furnished'; // Map to closest match
  ...
  void _onSave() {
    if (_formKey.currentState!.validate()) {
      ...
      if (_selectedMarket == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select Market'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_selectedBedrooms == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select No of Bedrooms'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_selectedBathrooms == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select No of Bathrooms'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_selectedFurnishing == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select Furnishing'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_selectedLocality == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select Locality'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_selectedViews == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select Views From Property'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_propertyImageUrl == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please upload Property Image'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_agreementUrl == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please upload Agreement'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (mode == PropertyMode.rental) {
        if (_selectedRentFrequency == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Please select Rent Frequency'),
              backgroundColor: Colors.red,
            ),
          );
          return;
      ...
      if (mode == PropertyMode.offPlan) {
        if (_handoverController.text.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Please select Handover Date'),
              backgroundColor: Colors.red,
            ),
          );
          return;
      ...
      if (_selectedDeveloperName == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please select Developer Name'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      final newProperty = NewProperty(
        ...
        rentFrequency: mode == PropertyMode.rental
            ? _selectedRentFrequency
            : null,
        paymentCount: _paymentCountController.text,
        price: _priceController.text.isNotEmpty ? _priceController.text : null,
        handover:
            mode == PropertyMode.offPlan && _handoverController.text.isNotEmpty
        ...
        otherCharges: _otherChargesController.text.isNotEmpty
            ? _otherChargesController.text
            : null,
        penalties: _penaltiesController.text.isNotEmpty
      ...
      if (isEditMode) {
        ref
            ...
            .update(widget.occupantRecordId!, newProperty)
            .then((_) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          isEditMode ? t('Edit Property') : t('Add'),
          ...
          Form(
            ...
            child: ListView(
              ...
              children: [
                ...
                if (selectedMode == PropertyMode.rental)
                  ..._buildRentalFields(t),
                if (selectedMode == PropertyMode.offPlan)
                  ..._buildOffPlanFields(t),

                _CustomDropdownField(
                  label: t('No of Bedrooms'),
                ...
                _CustomDropdownField(
                  label: t('Views From Property'),
                  value: _selectedViews,
                  items: _viewOptions,
                  onChanged: (v) => setState(() => _selectedViews = v),
                  translate: t,
                ),
                ...
                _buildAgreementUpload(
                  selectedMode == PropertyMode.rental
                      ? 'Rental Agreement'
                      : 'Off-Plan Agreement',
                ),
  ...
  List<Widget> _buildRentalFields(String Function(String) t) {
    return [
      ...
      _CustomDropdownField(
        ...
        items: _rentFrequencies,
        onChanged: (v) => setState(() => _selectedRentFrequency = v),
        translate: t,
      ),
      _CustomTextField(
          ...
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                color: selectedMode == PropertyMode.rental
                    ? Colors.yellow[700]
                    : Colors.grey[800],
    ...
    return Row(
      children: [
        _ModeChip(
          ...
          onTap: () => ref
              .read(propertyModeProvider.notifier)
              .setMode(PropertyMode.rental),
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            onChanged: onChanged,
            validator: (value) =>
                (value == null) ? t('Please make a selection') : null,
            style: const TextStyle(color: Colors.white),
            isExpanded: true,
            decoration: InputDecoration(
              filled: true,
...
class _CustomDatePickerField extends StatelessWidget {
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null) {
      controller.text = "${picked.toLocal()}".split(
        ' ',
